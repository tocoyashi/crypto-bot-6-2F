...
SYMBOLS = [
    ...
    "FET/USDT", "PEPE/USDT", "RENDER/USDT",
    "TON/USDT", "ETC/USDT", "HBAR/USDT", "ICP/USDT", "VET/USDT",
    "ALGO/USDT", "FTM/USDT"
]
...
def fetch_all_ohlcv(exchange, symbol, timeframe, since):
    """Fetch all OHLCV data in batches from 'since' to now."""
    all_data = []
    current_since = since
    while True:
...
def run_backtest():
    ...
    for symbol in SYMBOLS:
        try:
            ohlcv_15m = fetch_all_ohlcv(exchange, symbol, "15m", since)
            ohlcv_1h = fetch_all_ohlcv(exchange, symbol, "1h", since)
            ...
            for i in range(100, len(df_15m) - 96): 
                ...
                if outcome == "": 
                    ...
                    if direction == "LONG":
                        price_change = ((last_close / curr_close) - 1) * 100
                    else:
                        price_change = ((curr_close / last_close) - 1) * 100
                    
                    if state == 0:
                        # Full position still open — close all at last price
                        final_pnl += price_change + CLOSE_FEE
                    else:
                        # 65% closed at TP1 — close remaining 35% at last price
                        remaining_pos = 1.0 - CLOSE_TP1
                        final_pnl += (remaining_pos * price_change) + (remaining_pos * CLOSE_FEE)
                    timeouts += 1 
    ...
    print("\n" + "="*60)
    print("📊 BACKTEST REPORT — 3 MONTHS (65% / 35% PARTIAL CLOSE & FEES)")
    print("="*60)
    ...
    print(f"✅ Hit TP1 (Close 65%): {tp1_hits} ({tp1_rate:.1f}%)")
    print(f"📈 Hit TP2 (Close 35%): {tp2_hits} ({tp2_rate:.1f}%)")
    print("-" * 60)
